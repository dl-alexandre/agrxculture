...
const REPOSITORY = process.env.GITHUB_REPOSITORY || 'agrxculture/agricultural-portfolio-website';
...
async function getLastSuccessfulDeployment() {
  ...
  if (!GITHUB_TOKEN) {
    log('GitHub token not available, cannot fetch deployment history', 'warning');
    return null;
  }
  ...
  try {
    ...
    log(`Found last successful deployment: ${lastSuccessful.head_sha.substring(0, 7)} from ${lastSuccessful.created_at}`, 'success');
...
async function rollbackToCommit(commitSha) {
  ...
  try {
    ...
    const result = runCommand(`git push origin ${rollbackBranch}:gh-pages --force`);
