...
test.describe('Services Section Functionality', () => {
  ...
  test('services page loads with all services displayed', async ({ page }) => {
    ...
    await expect(page.locator('h2').first()).toContainText(
      'Agrxculture Technology Services'
    );
    ...
    await expect(
      page
        .locator('.service-card h3')
        .filter({ hasText: 'Custom IoT Integrations for Small Farms' })
    ).toBeVisible();
    await expect(
      page
        .locator('.service-card h3')
        .filter({ hasText: 'Mobile Farm Management Apps' })
    ).toBeVisible();
    await expect(
      page
        .locator('.service-card h3')
        .filter({ hasText: 'Agricultural Data Analytics' })
    ).toBeVisible();
    await expect(
      page
        .locator('.service-card h3')
        .filter({ hasText: 'Precision Agriculture APIs' })
    ).toBeVisible();
    await expect(
      page
        .locator('.service-card h3')
        .filter({ hasText: 'Farm Automation Systems' })
    ).toBeVisible();
  });

  test('service navigation links work correctly', async ({ page }) => {
    // Check that quick navigation links exist
    const navLinks = page.locator('.service-card .service-cta');
  ...
  test('service CTA links are functional', async ({ page }) => {
    ...
    const ctaButtons = page.locator('.service-cta');
  ...
  test('service detail page navigation works', async ({ page }) => {
    ...
    if ((await breadcrumbLink.count()) > 0) {
      ...
      test.skip(
        true,
        'Breadcrumb navigation not implemented on this service page'
      );
    }
  });
  ...
  test('contact CTA section is functional', async ({ page }) => {
    ...
    if ((await contactCta.count()) > 0) {
      ...
      test.skip(true, 'Contact CTA section not implemented on this page');
    }
  });
